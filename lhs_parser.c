...
#define LHS_EXPRREFER                         (32)
#define LHS_EXPRPUSHED                        (64)
...
#define LHS_EXPRSTR                           (4 | LHS_EXPRREFER)
#define LHS_EXPRSUB                           (5 | LHS_EXPRPUSHED)
#define LHS_EXPRCALL                          (6 | LHS_EXPRPUSHED)
#define LHS_EXPRTABLE                         (7 | LHS_EXPRPUSHED)
#define LHS_EXPRINDEX                         (8 | LHS_EXPRPUSHED)
...
#define lhsparser_exprcondiandcheckex(vm, lf, n, s, e)                              \
{                                                                                   \
...
lhsparser_exprcondiandcheckex(vm, lf, n, s, ((void*)(0)))
...
#define lhsparser_opcalls(vm, lf, l, c, n, a, w)                                      \
lhscode_op2((vm), &lhsparser_castlex(lf)->curfunction->code, OP_CALLS, (l), (c), (n));\
lhsparser_checkexprstack((vm), (lf), OP_CALL, (a) - (w) + 1)
...
static int lhsparser_checkexprstack(LHSVM* vm, LHSLoadF* loadf, char op, int call)
{
    ...
    case OP_SETTAB:
    {
        lhsparser_castlex(loadf)->nstack -= 2;
        break;
    }
    case OP_PUSH:    
    case OP_RET:
    ...
    (lhsparser_castlex(loadf)->nstack < 0) &&
        ...
        (
            vm, 
            loadf, 
            "illegal expression stack, at token '%s'.", 
            lhsparser_castlex(loadf)->token.buf.data
        );
    return LHS_TRUE;
}
...
static int lhsparser_exprmov(LHSVM* vm, LHSLoadF* loadf, LHSExprChain* chain)
{
    ...
    lhsparser_op1(vm, loadf, chain->type & LHS_EXPRREFER ? OP_MOV : OP_MOVS, chain);
    lhsparser_ref(vm, loadf, chain->prev->factor.ref.mark, chain->prev->factor.ref.index);

    if (chain->type & LHS_EXPRREFER)
    {
        lhsparser_ref(vm, loadf, chain->factor.ref.mark, chain->factor.ref.index);
    }
    return LHS_TRUE;
...
static int lhsparser_exprcall(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    ...
    lhsparser_checkandnexttoken(vm, loadf, LHS_TOKENIDENTIFIER, "call", "<identifier>");
    ...
    lhsparser_checkandnexttoken(vm, loadf, '(', "call", "(");
...
static int lhsparser_exprindexcall(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    /*indexcall -> '(' [exprargs] ')'*/
    lhsparser_checkandnexttoken(vm, loadf, '(', "calls", "(");

    char mark = LHS_MARKNONE, nwant = LHS_VOID;
    int index = 0, refer = lhsparser_castlex(loadf)->curfunction->name;
    int narg = lhsparser_exprargs(vm, loadf, state);
    ...
    lhsparser_opcalls(vm, loadf, loadf->line, loadf->column, refer, narg, nwant);
    lhsparser_index(vm, loadf, narg);
    lhsparser_byte(vm, loadf, nwant);
...
static int lhsparser_exprindex(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    /*exprindex -> LHS_TOKENIDENTIFIER {'[' exprstate ']' [indexcall] | ['.'] LHS_TOKENIDENTIFIER [indexcall]}*/
    lhsparser_checktoken(vm, loadf, LHS_TOKENIDENTIFIER, "index", "<identifier>");
    ...
    while (LHS_TRUE)
    {
        ...
        {
            lhsparser_exprindexcall(vm, loadf, state);
        }
        else
        {
            break;
...
static int lhsparser_exprtblargs(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    /*exprtblargs -> 
    {
        LHS_TOKENIDENTIFIER '=' exprstate   [','] | 
        '[' exprstate ']' '=' exprstate     [','] |
        exprstate                           [',']
    }
    */
    ...
    do
    {
        ...
        case LHS_TOKENIDENTIFIER:
        {
            ...
            lhsvm_pushlstring(vm, token->buf.data, token->buf.usize);
            LHSVar* name = lhsparser_insertconstant(vm, loadf);
            char mark = name->desc->mark; int index = name->desc->index;
            name = 0;

            lhsparser_nexttoken(vm, loadf);
            lhsparser_nexttoken(vm, loadf);

            lhsparser_exprcondiandcheck(vm, loadf, 1, "illegal table initializer.");
            lhsparser_op2(vm, loadf, OP_PUSH, loadf->line, loadf->column, index);
            lhsparser_ref(vm, loadf, mark, index);
            lhsparser_op2(vm, loadf, OP_SETTAB, loadf->line, loadf->column, index);
            expr = LHS_FALSE;
            break;
        }
        case '[':
        {
            lhsparser_nexttoken(vm, loadf);
            lhsparser_exprcondiandcheck(vm, loadf, 1, "illegal table initializer.");
            lhsparser_checkandnexttoken(vm, loadf, ']', "table", "]");
            lhsparser_checkandnexttoken(vm, loadf, '=', "table", "=");
            lhsparser_exprcondiandcheck(vm, loadf, 1, "illegal table initializer.");
            lhsparser_op2(vm, loadf, OP_SWAP, loadf->line, loadf->column, 
                lhsparser_castlex(loadf)->curfunction->name);
            lhsparser_op2(vm, loadf, OP_SETTAB, loadf->line, loadf->column, 
                lhsparser_castlex(loadf)->curfunction->name);
            expr = LHS_FALSE;
            break;
        }
        ...
        {
            lhsparser_exprcondiandcheck(vm, loadf, 1, "illegal table initializer.");
            lhsparser_op2(vm, loadf, OP_INSTAB, loadf->line, loadf->column, 
                lhsparser_castlex(loadf)->curfunction->name);
        }
    } while (token->t == ',' ? lhsparser_nexttoken(vm, loadf) : LHS_FALSE);

    return LHS_TRUE;
}

static int lhsparser_exprtable(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    /*exprtable -> '{' '}' | '{' exprtblargs '}'*/
    lhsparser_checkandnexttoken(vm, loadf, '{', "table", "{");

    state->chain->type = LHS_EXPRTABLE;
    state->chain->line = loadf->line;
    state->chain->column = loadf->column;
    state->chain->refer = lhsparser_castlex(loadf)->curfunction->name;
    state->chain->factor.ref.mark = LHS_MARKNONE;
    state->chain->factor.ref.index = 0;

    lhsparser_op1(vm, loadf, OP_PUSHTAB, state->chain);
    ...
    lhsparser_exprtblargs(vm, loadf, state);
    lhsparser_checkandnexttoken(vm, loadf, '}', "table", "}");
    return LHS_TRUE;
}
...
static int lhsparser_exprfactor(LHSVM* vm, LHSLoadF* loadf, LHSExprState* state)
{
    ...
    case LHS_TOKENIDENTIFIER:
    {
        ...
        if (lookahead->t == '(')
        {
            ...
            lhsparser_exprcode(vm, loadf, state->chain);
                 ...
                 lookahead->t == '.')
        {
            lhsparser_exprindex(vm, loadf, state);
            lhsparser_exprcode(vm, loadf, state->chain);
        }
        else
        {
            ...
            state->chain->type = LHS_EXPRREFER;
            state->chain->line = loadf->line;
            state->chain->column = loadf->column;
    ...
    case '{':
    {
        ...
        lhsparser_exprtable(vm, loadf, state);
        lhsparser_exprcode(vm, loadf, state->chain);        
        break;
    }
    default:
...
static int lhsparser_forarg1(LHSVM* vm, LHSLoadF* loadf, LHSForState* state)
{
    ...
    default:
    {
        lhsparser_exprcondiandcheck(vm, loadf, 0, "illegal for initializer.");
        break;
    }
    }
